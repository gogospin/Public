...
function [de,out] = my_CGrecon_nob0_acc_loopi_gpu(u,k,s,p,jump_rate,maxit,beta)
...
ps = func_AH_EH(k,p,s,u,jump_rate);
tt = toc(tstart);
...
fbar = waitbar(0,'ITERATION in progress');
for nit = 1:maxit
    
    %tstart = tic;
    
    %%qt = A(ps);
    qt = func_A_E(k,p,s,ps,nCoil,jump_rate);
    qt = reshape(qt,[],nCoil);
   ...
   % tstart = tic;
    %%qs = AH(qt);
    qs = func_AH_EH(k,p,s,qt,jump_rate);
%     tt = toc(tstart);
%     fprintf('iteration %d EH takes %d seconds\n',nit,tt);
%     clear tt tstart
    waitbar(nit/maxit,fbar,'ITERATION in progress');
    %qs = squeeze(sum(q,2));
    qs = qs+beta*ps;
